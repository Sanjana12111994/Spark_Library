...
class ForwardFillImputer(HasInputOutputCols, HasOrderByCols, HasListPartitionByCols):
    """
    An imputation class to impute by forward fill either for a single column, or by partitions.
    ...
    >>> ffill_imputer = ForwardFillImputer(input_cols=["Balance"], output_cols=["BalanceImp"], order_by_cols=["Date"])
    >>> ffill_imputer.impute(df).show()
    +----------+----------+-------+----------+
    |CustomerID|      Date|Balance|BalanceImp|
    +----------+----------+-------+----------+
    |     10001|2020-01-01|    145|       145|
    |     10002|2020-01-01|    474|       474|
    |     10001|2020-01-02|   null|       474|
    |     10002|2020-01-02|   null|       474|
    |     10001|2020-01-03|    155|       155|
    |     10002|2020-01-03|    356|       356|
    +----------+----------+-------+----------+

    >>> ffill_imputer.list_partition_by_cols = [["CustomerId"]]
    >>> ffill_imputer.impute(df).orderBy("Date", "CustomerId").show()
    +----------+----------+-------+----------+
    |CustomerID|      Date|Balance|BalanceImp|
    +----------+----------+-------+----------+
    |     10001|2020-01-01|    145|       145|
    |     10002|2020-01-01|    474|       474|
    |     10001|2020-01-02|   null|       145|
    |     10002|2020-01-02|   null|       474|
    |     10001|2020-01-03|    155|       155|
    |     10002|2020-01-03|    356|       356|
    +----------+----------+-------+----------+
    ...
    def impute(self, df: DataFrame) -> DataFrame:
        ...
        for i in range(len(self.input_cols)):
            ...
            f_filled_column = F.last(df[in_col], ignorenulls=True).over(w)

            df = df.withColumn(out_col, f_filled_column)
